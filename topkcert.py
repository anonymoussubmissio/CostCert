...
from utils.topkcert_util import certified_drs_four_delta, certified_drs_three_position, \
    ...
    certified_drs_pg_version_ablated, certified_drs_pg_version_exactly, certified_drs_pg_version_exactly_outputlabel
...
parser.add_argument('--dataset', default='imagenet', type=str,
                    choices=('imagenette', 'imagenet', 'cifar', 'cifar100', 'svhn', 'flowers102',"gtsrb"), help="dataset")
...
parser.add_argument("--patch_size", default=36, type=int, help="size of the adversarial patch (square patch)")
...
parser.add_argument("--dump_dir", default='dump620', type=str, help='directory to dump two-mask predictions')
...
parser.add_argument("--pg_true", type=bool, default=True, help='override dumped file')
parser.add_argument("--ablation_type", default="column", type=str)
parser.add_argument("--top", default=5, type=int, help="top-k")
parser.add_argument("--gpu", default=0, type=int, help="top-k")
...
# DRS_MODEL_NAME=DRS_MODEL_NAME+'_'+args.dataset+'_drs_'+str(args.ablation_size)+'_'+args.dataset+'_'+args.ablation_type
...
pg_cert_top1=0
drs_cert_top1=0
if args.pg_true==True:
    all_correct = np.zeros(shape=(NUM_IMG, 6))
else:
...
for i, (label, prediction_map_drs) in enumerate(
    ...
    pred, cnt = np.unique(prediction_map_drs, return_counts=True)
    # if len(pred)==1 and pred[0]==label:
    #     print(i)
    #     counter=counter+1
    ...
    #     continue
    T1 = time.time()
    position_benign_pg, robust_drs_pg_rank = certified_drs_pg_version_ablated(total_num_class, prediction_map_drs, ablation_size,
                                                                          patch_size, label)
    T2 = time.time()
    position_benign_new, robust_drs_new_rank = certified_drs_new_version(total_num_class, prediction_map_drs, ablation_size,
                                                                          patch_size, label)
    if args.pg_true:
        _, robust_drs_pg_true_rank = certified_drs_pg_version_exactly(total_num_class, prediction_map_drs, ablation_size,
                                                                          patch_size, label)
        position_benign_pg_true = certified_drs_pg_version_exactly_outputlabel(total_num_class, prediction_map_drs, ablation_size, patch_size, label)
        assert position_benign_pg_true<=robust_drs_pg_true_rank
    assert position_benign_new<=robust_drs_new_rank and position_benign_pg<=robust_drs_pg_rank
    ...
    if args.pg_true:
        all_correct[i]=[position_benign_pg, robust_drs_pg_rank, position_benign_new, robust_drs_new_rank, position_benign_pg_true[0], robust_drs_pg_true_rank]
    else:
    ...
    if args.pg_true:
        if robust_drs_pg_true_rank<=TOP_K:
            robust_drs_pgtrue_bool = True
        else:
            robust_drs_pgtrue_bool = False
    ...
    if args.pg_true:
        if position_benign_pg_true<=TOP_K:
            pgtrue_correct_bool = True
        else:
            pgtrue_correct_bool = False
    ...
    if robust_drs_pg_bool and pg_correct_bool:
        pg_cert=pg_cert+1
    if args.pg_true:
        if robust_drs_pgtrue_bool and pgtrue_correct_bool:
            pgtrue_cert=pgtrue_cert+1
    ...
    if args.pg_true:
        if pgtrue_correct_bool:
            pgtrue_correct=pgtrue_correct+1
    # end
    ...
    # print("new_correct " + str(new_correct) + ' ' + str(new_correct / NUM_IMG))
    print("pgtrue_cert " + str(pgtrue_cert) + ' ' + str(pgtrue_cert / NUM_IMG))
    ...
    print("pgtrue_correct " + str(pgtrue_correct) + ' ' + str(pgtrue_correct / NUM_IMG))
    print("\n")
print(T_TC)
print(T_PG)
print(counter/len(prediction_map_list_drs))
# all_correct = all_correct[~np.all(all_correct == 0, axis=1)]
# top_not_1_save=top_not_1_save[~np.all(top_not_1_save == 0, axis=1)]

np.savetxt("./6_23_top_k_list_pg_and_topkcert_{}_{}_{}_{}_{}.csv".format(DATASET, DRS_MODEL_NAME,ablation_size,ablation_type, patch_size), all_correct,fmt="%.2f",delimiter=",")
