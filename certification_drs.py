...
MODEL_NAME=MODEL_NAME+'_'+args.dataset+'_drs_'+str(args.ablation_size)+'_'+args.dataset+'_'+args.ablation_type
...
if False:
    ...
    for i, (data, labels) in enumerate(tqdm(val_loader)):
        if i==203:
            ...
            plt.margins(0, 0)
            plt.imshow((data.cuda()).cpu()[0].permute(1, 2, 0))
            plt.show()
        data = data.to(device)
        labels = labels.numpy()
...
# vit_base_patch16_224_gtsrb_drs_19_gtsrb_column_gtsrb.pth
# vit_base_patch16_224_cifar_drs_19_cifar_column_cifar_drs_19_cifar_column_cifar.pth